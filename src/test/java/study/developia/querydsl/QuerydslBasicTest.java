...
//        QMember m = QMember.member;
    ...
    void searchAndParam() {
        Member findMember = queryFactory
                .selectFrom(member)
                .where(
                        // and인경우 ,만 해도 됨 (null이 들어갔을 때 무시됨)
...
//                .fetchOne();
    ...
    //문자 더하기
    //stringValue()를 통해서 문자 처리, enum값 처리할 때도 사용
    @Test
    void concat() {
        //{username}_{age}
    ...
    void findDtoByJPQL() {
        //new operation
        List<MemberDto> result = em.createQuery("select new study.developia.querydsl.dto.MemberDto(m.username, m.age)" +
...
//    @Commit
    void bulkUpdate() {
        ...
        /**
         * 벌크 연산의 경우에는 영속성 컨텍스트를 무시하고 db에 반영하기 때문에
         * 영속성 컨텍스트와 값이 다르기 때문에 주의해야함.
         * 만약 이렇게 다른 경우라고 해도 영속성 컨텍스트가 자동 업데이트 되지 않는다.
         */
        ...
        // 그렇기 때문에 벌크 연산은 연산 후 영속성 컨텍스트를 초기화 해야함
        em.flush();
        em.clear();
